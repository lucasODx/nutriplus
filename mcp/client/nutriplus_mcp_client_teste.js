...
/**
 * Substitui o AsyncExitStack do Python: guarda funções de finalização
 * e as executa todas, na ordem inversa, quando closeAll() é chamado.
 */
function criarStack() {
  const finalizadores = [];
  return {
...
async function conectarServicos(stack) {
  ...
  for (const [nomeServico, url] of Object.entries(SERVICOS_MCP)) {
    ...
    const conexao = new Client({ name: "kkkboombot", version: "1.0.0" });
    ...
    while (true) {
      ...
      while (true) {
        ...
        for (const execucao of execucoes) {
          console.log("🔍 function_call recebido:", JSON.stringify(execucao, null, 2));

          const argumentos = execucao.arguments;
          const resultado = await executarFerramenta(ferramentas, execucao.name, argumentos);

          console.log("🔍 resultado da ferramenta (texto enviado ao modelo):", resultado);
          ...
          console.log("🔍 function_result enviado:", JSON.stringify(functionResult, null, 2));

          mensagens.push(functionResult);
        }
      }
    }
  } finally {
